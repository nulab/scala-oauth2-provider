...
trait ProtectedResource {
  ...
  )(implicit ctx: ExecutionContext): Future[Either[OAuthError, AuthInfo[U]]] =
    try {
      fetchers
        .find { fetcher =>
          ...
          handler
            ...
            .flatMap { maybeToken =>
              ...
              handler
                ...
                .map(
                  _.map(Right(_)).getOrElse(
                    Left(new InvalidToken("The access token is invalid"))
                  )
                )
            }
            .recover {
