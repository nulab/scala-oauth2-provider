...
class AuthHeaderSpec extends AnyFlatSpec {
  ...
  def createRequest(authorization: Option[String]): ProtectedResourceRequest =
    authorization match {
      case Some(s) =>
        new ProtectedResourceRequest(Map("Authorization" -> Seq(s)), Map())
  ...
  it should "fetch parameter from OAuth" in {
    val result = AuthHeader.fetch(
      createRequest(
        Some(
          """OAuth access_token_value,algorithm="hmac-sha256",nonce="s8djwd",signature="wOJIO9A2W5mFwDgiDvZbTSMK%2FPY%3D",timestamp="137131200""""
        )
      )
    )
  ...
  it should "fetch parameter from Bearer" in {
    val result = AuthHeader.fetch(
      createRequest(
        Some(
          """Bearer access_token_value,algorithm="hmac-sha256",nonce="s8djwd",signature="wOJIO9A2W5mFwDgiDvZbTSMK%2FPY%3D",timestamp="137131200""""
        )
      )
    )
    result.token should be("access_token_value")
  ...
  it should "fetch illegal parameter without =" in {
    val result = AuthHeader.fetch(
      createRequest(Some("""OAuth access_token_value,fizz=buzz,foo"""))
    )
    result.token should be("access_token_value")
