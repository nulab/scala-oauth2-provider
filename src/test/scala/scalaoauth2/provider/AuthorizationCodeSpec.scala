...
class AuthorizationCodeSpec
  ...
  it should "handle request" in {
    ...
    val f = authorizationCode.handleRequest(
      clientCred,
      request,
      new MockDataHandler() {
    ...
    whenReady(f, timeout(Span(1, Seconds)), interval(Span(50, Millis))) {
      result =>
  ...
  it should "handle a PKCE plain (implicit) request" in {
    ...
    val request = new AuthorizationRequest(
      ...
      Map(
        ...
        "code" -> Seq("code1"),
        "redirect_uri" -> Seq("http://example.com/"),
        "code_verifier" -> Seq("4g94A5mKbcP1zv313x6JmVQjDJ1FiwVFBnLepwk1BLk")
      )
    )
    val clientCred = request.parseClientCredential
      .fold[Option[ClientCredential]](None)(_.fold(_ => None, c => Some(c)))
          ...
          Future.successful(
            Some(
              AuthInfo(
                ...
                redirectUri = None,
                codeChallenge =
                  Some("4g94A5mKbcP1zv313x6JmVQjDJ1FiwVFBnLepwk1BLk")
              )
            )
          )
          ...
          Future.successful(
            Some(
              AuthInfo(
                ...
                redirectUri = None,
                codeChallenge =
                  Some("aLE640XFc4YAPoVB3KCUhcjoMRDQhyILWy9k9qpiBfo"),
  ...
  it should "return a InvalidGrant if PKCE validation fails for equality" in {
    ...
    val request = new AuthorizationRequest(
      ...
      Map(
        ...
        "code" -> Seq("code1"),
        "redirect_uri" -> Seq("http://example.com/"),
        "code_verifier" -> Seq("iB3OM4lYP6k03yT_sMGr1o_Mf-lOX84mrM7jRkm21Ak")
      )
    )
    val clientCred = request.parseClientCredential
      .fold[Option[ClientCredential]](None)(_.fold(_ => None, c => Some(c)))
