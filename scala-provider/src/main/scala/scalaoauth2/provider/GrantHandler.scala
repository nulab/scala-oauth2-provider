...
trait GrantHandler {
  ...
  def handleRequest(request: Request, dataHandler: DataHandler): GrantHandlerResult

  def issueAccessToken(dataHandler: DataHandler, authInfo: AuthInfo): GrantHandlerResult = {
...
class RefreshToken(clientCredentialFetcher: ClientCredentialFetcher) extends GrantHandler {
  ...
  override def handleRequest(request: Request, dataHandler: DataHandler): GrantHandlerResult = {
    ...
    val newAuthInfo = dataHandler.createOrUpdateAuthInfo(authInfo.userId, authInfo.clientId, authInfo.scope).getOrElse(authInfo)
...
class Password(clientCredentialFetcher: ClientCredentialFetcher) extends GrantHandler {
  ...
  override def handleRequest(request: Request, dataHandler: DataHandler): GrantHandlerResult = {
    ...
    val userId = dataHandler.findUserId(username, password).getOrElse(throw new InvalidGrant())
    val scope = request.param("scope")
    val clientId = clientCredential.clientId
    val authInfo = dataHandler.createOrUpdateAuthInfo(userId, clientId, scope).getOrElse(throw new InvalidGrant())
...
class ClientCredentials(clientCredentialFetcher: ClientCredentialFetcher) extends GrantHandler {
  ...
  override def handleRequest(request: Request, dataHandler: DataHandler): GrantHandlerResult = {
    ...
    val clientId = clientCredential.clientId
    val userId = dataHandler.findClientUserId(clientId, clientSecret).getOrElse(throw new InvalidGrant())
    val scope = request.param("scope")
    val authInfo = dataHandler.createOrUpdateAuthInfo(userId, clientId, scope).getOrElse(throw new InvalidGrant())
